...
def get_links(dbo, pid):
    ...
    sql = "SELECT 'OO' AS TYPE, " \
        ...
        % ( _("Original Owner", l), linkdisplay, animalextra, int(pid), 
        _("Brought In By", l), linkdisplay, animalextra, int(pid),
        _("Picked Up By", l), linkdisplay, animalextra, int(pid),
        _("Owner Vet", l), linkdisplay, int(pid), 
        _("Current Vet", l), linkdisplay, int(pid),
        _("Waiting List Contact", l), int(pid), 
        _("Lost Animal Contact", l), int(pid),
        _("Found Animal Contact", l), int(pid),
        _("Animal Control Incident", l), int(pid), int(pid), int(pid), 
        _("Animal Control Caller", l), int(pid), 
        _("Animal Control Victim", l), int(pid) )
    return db.query(dbo, sql)
...
def get_person_find_simple(dbo, query, classfilter="all", includeStaff = False, limit = 0, onlyindexed = False):
    """
    ...
    if not onlyindexed:
        ...
        ors.append(add("o.AdditionalFlags"))
        ors.append(u"EXISTS(SELECT ad.Value FROM additional ad WHERE ad.LinkID=o.ID AND ad.LinkType IN (%s) AND LOWER(ad.Value) LIKE '%%%s%%')" % (additional.PERSON_IN, query.lower()))
    cf = ""
    sf = ""
    if classfilter == "all":
        pass
...
def get_person_find_advanced(dbo, criteria, includeStaff = False, limit = 0):
    ...
    criteria: A dictionary of criteria
       ...
       filter - "all" "aco" "banned" "donor" "fosterer" "homechecked"
...
def update_person_from_form(dbo, post, username):
    ...
    sql = db.make_update_user_sql(dbo, "owner", username, "ID=%d" % pid, (
        ...
        ( "OwnerName", db.ds("%s %s %s" % ( post["title"], post["forenames"], post["surname"] ))),
...
def lookingfor_report(dbo, username = "system"):
    ...
    for p in people:
        ...
        if len(c) > 0:
            h.append( "<p style='font-size: 8pt'>(%s: %s)</p>" % (_("Looking for", l), ", ".join(c)))
